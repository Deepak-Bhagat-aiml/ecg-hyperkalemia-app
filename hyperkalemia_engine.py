...
class HyperkalemiaPredictor:
    ...
    def parse_header(self, filepath_or_buffer):
        ...
        try:
            if isinstance(filepath_or_buffer, str):
                ...
                lines = [filepath_or_buffer.readline().decode("utf-8", errors="ignore") for _ in range(20)]
                filepath_or_buffer.seek(pos)

            for line in lines:
                line = line.strip()
                if line.startswith("#"):
                    possible_json = line.lstrip("#").strip()
                    if possible_json.startswith("{"):
                        try:
                            meta = json.loads(possible_json)
                            dev_key = list(meta.keys())[0]
                            info = meta[dev_key]
                            columns = info.get("column", columns)
                            sampling_rate = info.get("sampling rate", sampling_rate)
                            resolution = info.get("resolution", [16])
            ...
            print("Header parsing fallback:", e)
    ...
    def load_ecg_signal(self, file_path_or_buffer, vcc=3.0, gain=1000.0):
        ...
        is_timestamp_col = any("timestamp" in str(c).lower() for c in columns)
        ...
        if is_timestamp_col or max_abs < 50.0:
            ...
            dt = dt[dt > 0]
            if len(dt) > 0:
                estimated_fs = 1000.0 / np.median(dt)
                if 50 <= estimated_fs <= 5000:
                    fs = estimated_fs
                else:
                    fs = 500.0 if len(ecg_raw) <= 4500 else fs_declared
            else:
                fs = 500.0 if len(ecg_raw) <= 4500 else fs_declared
        else:
    ...
    def preprocess_signal(self, ecg_mv, fs_source, target_fs=500.0):
        ...
        # 1000 Hz -> 500 Hz
        if abs(fs_source - 1000.0) < 1e-6:
            return sp_signal.resample_poly(ecg_mv, up=1, down=2)

        # Generic resampling
        n_target = int(round(len(ecg_mv) * target_fs / fs_source))
        return sp_signal.resample(ecg_mv, n_target)
    ...
    def scale_windows(self, windows):
        ...
        n_feat = getattr(self.scaler, "n_features_in_", 1) if self.scaler is not None else 1
        ...
        if n_feat == 1:
            ...
            if self.scaler is not None:
                ...
                scaled = (flat - DEFAULT_SCALER_MEAN) / DEFAULT_SCALER_SCALE
            return scaled.reshape(n_win, win_len, 1).astype(np.float32)
        else:
            flat = windows.reshape(n_win, win_len)
            scaled = self.scaler.transform(flat)
            return scaled.reshape(n_win, win_len, 1).astype(np.float32)
